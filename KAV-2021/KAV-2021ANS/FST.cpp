...
void FST::LexAnalyzer(In::IN in, Out::OUT out, Log::LOG log, LT::LexTable& lextable, IT::IdTable& idtable) //выполнить распознование цепочки
{
	...
	libs[0].name = "ord";
	libs[0].params[0] = IT::CHR;
	libs[0].parcount = 1;
	libs[0].retval = IT::INT;

	libs[1].name = "chr";
	libs[1].params[0] = IT::INT;
	libs[1].parcount = 1;
	libs[1].retval = IT::CHR;

	libs[2].name = "GetMonth";
	libs[2].params[0] = IT::INT;
	libs[2].parcount = 1;
	libs[2].retval = IT::INT;

	libs[3].name = "GetHours";
...
#pragma region "Структура checker и её массив автоматов"
	...
	Checker checkArr[] = {
		...
		Checker(&l_let,LEX_DECLARE,(IT::IDDATATYPE)FALSYNUMBER),
